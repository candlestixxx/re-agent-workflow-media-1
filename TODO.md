...
21. **Real-time UI Dashboard**: Integrate `socket.io` into `src/index.ts` to push live pipeline execution updates to the frontend dashboard. (In Progress)
22. **Microservice Splitting**: Break apart monolithic express routes into specialized containerized sub-services. (Pending)