...
import { VideoProcessingJob } from '../src/models/VideoProcessingJob';

describe('VideoProcessingService', () => {
  let baseJob: VideoProcessingJob;

  beforeEach(() => {
    baseJob = VideoProcessingService.initializeJob(
      'list-123',
      'Coming Soon',
      'drone',
      '16:9'
  ...
  describe('updateJobStatus', () => {
    it('should transition to Editing correctly', () => {
      const updated = VideoProcessingService.updateJobStatus(baseJob, 'Editing');
    ...
    it('should transition to Completed only when outputPath is provided', () => {
      ...
      expect(() => VideoProcessingService.updateJobStatus(baseJob, 'Completed')).toThrow(
        'An outputPath must be provided when marking a video job as Completed'
      );

      // Ensure it passes when output path is provided
      const completed = VideoProcessingService.updateJobStatus(baseJob, 'Completed', '/mock/video.mp4');
    ...
    it('should throw an error if attempting to transition an already Completed job', () => {
      ...
      expect(() => VideoProcessingService.updateJobStatus(baseJob, 'Queued')).toThrow(
        'Cannot transition a job that is already Completed'
      );
    });

    it('should throw an error if attempting to transition a Failed job', () => {
      baseJob.status = 'Failed';
      expect(() => VideoProcessingService.updateJobStatus(baseJob, 'Editing')).toThrow(
        'Cannot transition a job that is already Failed'
      );
    });
  });
});
