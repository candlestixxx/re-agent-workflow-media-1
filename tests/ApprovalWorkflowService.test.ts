...
describe('ApprovalWorkflowService', () => {
  ...
  describe('submitForApproval', () => {
    it('should transition a Draft job to Pending_Approval', () => {
      const updated = ApprovalWorkflowService.submitForApproval(baseJob);
      expect(updated.status).toBe('Pending_Approval');
    });

    it('should throw an error if job is already Approved', () => {
      baseJob.status = 'Approved';
      expect(() => ApprovalWorkflowService.submitForApproval(baseJob)).toThrow(
        'Cannot submit job for approval from status: Approved'
      );
    });
  });
  ...
  describe('approveJob', () => {
    it('should transition a Pending_Approval job to Approved and record reviewer', () => {
      ...
      const updated = ApprovalWorkflowService.approveJob(baseJob, 'broker-1');
    ...
    it('should throw an error if the job is not in Pending_Approval status', () => {
      expect(() => ApprovalWorkflowService.approveJob(baseJob, 'broker-1')).toThrow(
        "Only jobs in 'Pending_Approval' can be approved"
      );
    });
  });
  ...
  describe('publishJob', () => {
    it('should transition an Approved job to Published', () => {
      ...
      const updated = ApprovalWorkflowService.publishJob(baseJob);
      expect(updated.status).toBe('Published');
    });

    it('should throw an error if trying to publish a Draft job', () => {
      expect(() => ApprovalWorkflowService.publishJob(baseJob)).toThrow(
        "A job must be 'Approved' before it can be published"
      );
    });
    ...
    it('should throw an error if job is Approved but lacks an approvedBy record', () => {
      ...
      expect(() => ApprovalWorkflowService.publishJob(baseJob)).toThrow(
        'A job cannot be published without a recorded approver'
      );
    });
  });
});
